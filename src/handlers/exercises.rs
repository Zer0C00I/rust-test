...
pub(super) fn register(window: &MainWindow, state: AppState) {
    ...
    window.on_add_exercise(move |program_id, name, sets, reps, weight, notes| {
        let db = db.borrow();
        let _ = db.add_exercise(program_id as i64, &name, sets as i64, reps as i64, weight as i64, &notes);
    ...
    window.on_update_exercise(move |id, name, sets, reps, weight, notes| {
        let db = db.borrow();
        let _ = db.update_exercise(id as i64, &name, sets as i64, reps as i64, weight as i64, &notes);
